...
#Importamos la serie del DAX30 y de futuros sobre el DAX30 para junio 2020 sobre la que vamos a trabajar
...
datos <- arrange(DAX30_FUT, date) #Invertimos el orden de los datos (ordenados por fecha).


##PARÁMETROS##

r <- -0.00214 #Tipo de interés medio del bono alemán a 10 años (31DIC18-31DIC19)
q <- 0.0379859 #Dividendos del DAX30 (proporcionado por Thomson Reuters a día 11 de abril de 2020)
...
datos$t_f <- as.Date(rep("2020-06-19", times = length(datos$date))) - as.Date(datos$date) #Días hasta vencimiento del futuro
t_f <- as.numeric(datos$t_f/250) #Anualizamos los días y los convertimos en números
datos$t <- as.Date(rep("2020-04-09", times = length(datos$date))) - as.Date(datos$date) #Días hasta vencimiento de la opción
...
nd1 <- pnorm(-d1)
nd2 <- pnorm(-d2)
p <- 1000*(X*exp(-r*t)*nd2 - S*exp(-q*t)*nd1) #Valoración de la put. Multiplicamos por 1000 porque el nominal es 1000 veces X
...
abline(v=X, col="blue") #Gráfico de delta sobre el subyacente.

plot(posc, xlab = "Días", ylab = "Posición") #Gráfico para observar la posición en el mercado de futuros.

plot(rdo, xlab = "Días", ylab = "Resultado") #Gráfico para observar el resultado de la cobertura.
abline(h=0, col = "blue")

ggdensity(track_err, xlab = "Tracking error", ylab = "Densidad") #Gráfico del tracking error.
